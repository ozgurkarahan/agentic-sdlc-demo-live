...
[CmdletBinding()]
param(
  [string]$Subscription   = 'b7865e8a-b27f-4ba5-9720-d6884ab5cda2',
...
function Enable-AcrPull($appName) {
  ...
  $principalId = az containerapp identity show -g $ResourceGroup -n $appName --query principalId -o tsv 2>$null
  if (-not $principalId) {
  ...
  $havePull = az role assignment list --assignee $principalId --scope $acrId --query "[?roleDefinitionName=='AcrPull'] | [0].id" -o tsv 2>$null
  if (-not $havePull) {
    for ($i = 0; $i -lt 6; $i++) {
      try { az role assignment create --assignee-object-id $principalId --assignee-principal-type ServicePrincipal --role AcrPull --scope $acrId --only-show-errors | Out-Null; break }
